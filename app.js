...
//app.post('/api/v1/upload', resources.v1.upload.post)
//app.get('/api/v1/upload', resources.v1.upload.get)
//66666666666666
var fs = require('fs');
...
var cur = new Date();
var uploadFolder = './uploads/' + cur.getFullYear() + "/" + cur.getMonth() + "/";
...
app.post('/upload', upload.single('imgData'), function (req, res, next) {
  ...
  var base64Data = imgData.replace(/^data:image\/\w+;base64,/, "");
    ...
    var photoName = "street-" + Date.now() +".png"
    fs.writeFile(uploadFolder + photoName, dataBuffer, function(err) {
        if(err){
          res.send(err);
        }else{
          res.send("保存成功！");
